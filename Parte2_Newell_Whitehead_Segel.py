...
'''
Este script resuelve la ecuacion de difusion-reaccion Newell-Whitehead-Segel
dn/dt=gamma d2n/dx2 + mu*n-mu*n3
...
# Se escoge la semilla para generar numeros aleatorios
seed = int(raw_input('elegir semilla: '))
np.random.seed(seed)
...
def inicializa_n(n, N_pasos, h):
    ...
    for i in range(N_pasos):
        n[i] = np.random.uniform(low=-0.3, high=0.3, size=N_pasos)[i]
    n[0] = 0
    n[-1] = 0
...
def calcula_b(b, N_pasos, r):
    ...
    for j in range(1, N_pasos - 1):
        b[j] = r * n[j+1] + (1-2*r) * n[j] + r * n[j-1] + dt*(n[j] - n[j]**3)
...
plt.title('densidad vs posicion para 0 < t < 15, semilla = ' + str(seed))
...
ax2.set_title('densidad en el espacio para 0 < t < 15, semilla = ' + str(seed))
